...
public class Asteroids extends Game {
	...
	private java.util.List<Bullet> beingRemoved = new ArrayList<Bullet>();
	...
	public void paint(Graphics brush) {
       ...
       if(collision) {
		...
		//changing the color of stars on and off to make them blink
		if (counter%10 == 0) {
			for (int i = 0; i < stars.length; i++) {
				if(i/2 == 0) {
					...
					stars[i].paint(brush, Color.DARK_GRAY);
				}
			}
		}
		
		ArrayList<Bullet> listBullets = ship.getBullets();
		...
		for (Bullet bullet : listBullets) {
			...
			if (bullet.outOfBounds()) {
				beingRemoved.add(bullet);
				System.out.println("bullet");
			}	
			//checking if the asteroid contains the bullet and putting the bullet to the beingRemoved list and removing the asteroid right away
			for (int i = 0; i < listBullets.size(); i++) {
				for (int j = 0; j < randomAsteroids.size(); j++) {
					if (randomAsteroids.get(j).contains(listBullets.get(i).getCenter())){
						randomAsteroids.remove(randomAsteroids.get(j));
						beingRemoved.add(listBullets.get(i));
					}
				}
			}
		}	

		listBullets.removeAll(beingRemoved);
		//System.out.println(listBullets.size());
